...
class PPOModel(nn.Module):
    ...
    def __init__(self, name, NubPhyPara, NubComPara, NubTimeSeq, ClipNetOut, ActCase=2):
        ...
        if LSTMMODE:
            ...
            self.FC2_A_mu = nn.Linear(8*(self.NubPhyPara + self.NubComPara), self.ActCase)    # default 2 out
            self.FC2_A_std = nn.Linear(8*(self.NubPhyPara + self.NubComPara), self.ActCase)    # default 2 out
    ...
    def GetPredictActorOut(self, x_py, x_comp):
        ...
        if self.ModelName == "Progno1":
            mu = torch.tanh(self.FC2_A_mu(x))
            # TOOL.ALLP(mu, comt='mu')
            std = nn.functional.softplus(self.FC2_A_std(x))
            # TOOL.ALLP(std, comt='std')
            return mu, std
        else:
