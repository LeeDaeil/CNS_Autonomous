...
class NETBOX:
    ...
    def __init__(self):
        self.NET = {
            ...
            0: PPODisActor(name="Pypara_A", NubPhyPara=3, NubComPara=2, NubTimeSeq=10, ActCase=3),
            1: PPODisCritc(name="Pypara_C", NubPhyPara=3, NubComPara=2, NubTimeSeq=10, ActCase=1),
            2: PPODisActor(name="Compara_A", NubPhyPara=3, NubComPara=2, NubTimeSeq=10, ActCase=2),
            3: PPODisCritc(name="Compara_C", NubPhyPara=3, NubComPara=2, NubTimeSeq=10, ActCase=1)
        }
...
class PPODisActor(nn.Module):
    def __init__(self, name, NubPhyPara, NubComPara, NubTimeSeq, ActCase=2):
        # 상속
        super(PPODisActor, self).__init__()
        # 입력 정보
        self.ModelName = name  # str
        self.NubPhyPara = NubPhyPara  # int
        self.NubComPara = NubComPara  # int
        ...
        # logstd
        self.logstd = nn.Parameter(torch.zeros(self.ActCase))
...
class PPODisCritc(nn.Module):
    def __init__(self, name, NubPhyPara, NubComPara, NubTimeSeq, ActCase=1):
        # 상속
        super(PPODisCritc, self).__init__()
        # 입력 정보
        self.ModelName = name  # str
        self.NubPhyPara = NubPhyPara  # int
        self.NubComPara = NubComPara  # int
...
if __name__ == '__main__':
    for net_name in ["TEST", "Progno1"]:
        ...
        # TESTMODEL = PPODisActor(name=net_name, NubPhyPara=3, NubComPara=2, NubTimeSeq=10, ActCase=2)
        TESTMODEL = PPODisCritc(name=net_name, NubPhyPara=3, NubComPara=2, NubTimeSeq=10, ActCase=1)
