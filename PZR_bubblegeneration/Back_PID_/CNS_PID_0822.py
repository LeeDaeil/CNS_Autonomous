...
class AnimationWidget(QWidget):
    def __init__(self, ip, port, cns_ip, cns_port, kp, ki, kd):
        ...
        self.pid.SetPoint_pres = 24.0
        # self.pid.SetPoint_temp = 100.0
        ...
        self.RHR_position = []  # RHR valve position list
        ...
        self.x11 = np.arange(18000)
        self.y11 = np.ones(18000, dtype=np.float) * np.nan
        self.line11, = self.canvas.axes11.plot(self.x11, self.y11, animated=True, color='blue', lw=2)
        self.x12 = np.arange(18000)
        self.y12 = np.ones(18000, dtype=np.float) * np.nan
        self.line12, = self.canvas.axes12.plot(self.x12, self.y12, animated=True, color='red', lw=2)

        self.x21 = np.arange(18000)
        self.y21 = np.ones(18000, dtype=np.float) * np.nan
        self.line21, = self.canvas.axes21.plot(self.x21, self.y21, animated=True, color='blue', lw=2)
        self.x22 = np.arange(18000)
        self.y22 = np.ones(18000, dtype=np.float) * np.nan
        self.y22 = np.ones(18000, dtype=np.float) * np.nan
    ...
    def update_line(self, i):
        ...
        # self.pid.SetPoint_temp = self.Subwindow.setpoint1
        ...
        pos1 = self.mem['BHV603']['V']  # charging valve position
        ...
        self.RHR_position.append(pos1)  # RHR valve position list
        ...
        if self.mem['ZINST65']['V'] >= 25:
            self.send_control(['KSWO58', 'KSWO59'], [0, 1])  # HV-603 open

        else:
                self.send_control(['KSWO58', 'KSWO59'], [1, 0])  # HV-603 close

        if self.mem['UUPPPL']['V'] >= 54:
            self.send_control(['KSWO130'], [1])  # RCP_2 OIL PUMP
            self.send_control(['KSWO133'], [1])  # RCP_2 ON

        if self.mem['UUPPPL']['V'] >= 82:
            self.send_control(['KSWO129'], [1])  # RCP_1 OIL PUMP
            self.send_control(['KSWO132'], [1])  # RCP_1 ON
            self.send_control(['KSWO131'], [1])  # RCP_3 OIL PUMP
            self.send_control(['KSWO134'], [1])  # RCP_3 ON

        # if self.mem['UUPPPL']['V'] >= 100:
        #     self.send_control(['KSWO125'], [1])  # Heater up
        #     self.send_control(['KSWO120', 'KSWO121', 'KSWO122'], [1, 0, 1])  # Heater up
        #
        # if self.mem['ZINST63']['V'] <= 80:
        #     self.send_control(['KSWO125'], [0])  # Heater off
        #     self.send_control(['KSWO120', 'KSWO121', 'KSWO122'], [1, 1, 0])  # Heater off
    ...
    # if prz_level <= 90:
        ...
        # if rcs_temp < 100:
        #     if valve_control >= 0.005:
        #         self.send_control(['KSWO90', 'KSWO91'], [1, 0])  # PV145 close
        #     elif abs(valve_control) < 0.005:
        #         self.send_control(['KSWO90', 'KSWO91'], [0, 0])  # PV145 no action
        #     else:
        #         self.send_control(['KSWO90', 'KSWO91'], [0, 1])  # PV145 Open
        ...
        y21 = pos2              # RHR Hx valve position
        old_y21 = self.line21.get_ydata()
        new_y21 = np.r_[old_y21[1:], y21]
        self.line21.set_ydata(new_y21)

        y22 = pos1              # letdown valve position
...
class MyMplCanvas(FigureCanvas):
    def __init__(self, parent=None, width=8, height=8, dpi=300):
        ...
        self.axes11 = fig.add_subplot(211, xlim=(0, 18000), ylim=(20.0, 30.0))
        ...
        self.axes12.set_ylim(40.0, 180.0)  # temperature plot limit
        self.axes12.set_ylabel('Temperature ($^o$C)', fontsize=10, color="red")
        # self.axes12.legend('temperature', loc='upper right', fontsize=10)

        self.axes21 = fig.add_subplot(212, xlim=(0, 18000), ylim=(-0.25, 1.25))
        self.axes21.grid(True)  # grid on
        self.axes21.set_ylabel('Letdown valve opening (%)', fontsize=10, color="blue")
        ...
        self.axes22.set_ylabel('RHR HX valve opening (%)', fontsize=10, color="red")
