from Pytorch_A3C.Net_Model_NO_proba import Agent_network
...
class Worker(mp.Process):
    ...
    def run(self):
        while True:
            ...
            for i in range(1000):
                ...
                # a, prob_a, ca, prob_ca = self.L_net.get_one_act(_input_state)
                prob_a, a, prob_ca, ca, value = self.L_net.get_one_act(_input_state)
                ...
                [Leg_s, Leg_r, Leg_a, Leg_ca] = self.W_info.append_buf(s=set_s, r=r, a=[a], ca=[ca])
    ...
    def push_and_pull(self, opt, lnet, gnet, done, s_, buf):
        ...
        loss = lnet.loss_fun(torch.FloatTensor(bs), torch.LongTensor(ba), torch.LongTensor(bca),
