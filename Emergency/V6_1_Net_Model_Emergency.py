...
class NETBOX:
    ...
    def __init__(self):
        self.NET = {
            ...
            1: PPOModel2(name="AuxA", NetType='A', NubPhyPara=6, NubComPara=7, NubTimeSeq=15, ClipNetOut=[0, 0.25], ActCase=3),
            2: PPOModel2(name="AuxC", NetType='C', NubPhyPara=6, NubComPara=7, NubTimeSeq=15, ClipNetOut=[-1, 1], ActCase=1),
        }
...
class PPOModel2(nn.Module):
    """
    Fun: 1개 네트워크 모델 구성하여 반환, Critic and Value
    """
    def __init__(self, name, NetType, NubPhyPara, NubComPara, NubTimeSeq, ClipNetOut, ActCase=2):
        # 상속
        super(PPOModel2, self).__init__()
        # 입력 정보
        self.ModelName = name           # str
        self.NetType = NetType          # str
        self.NubPhyPara = NubPhyPara    # int
        self.NubComPara = NubComPara    # int
        self.NubTimeSeq = NubTimeSeq    # int
        self.ClipNetOut = ClipNetOut    # [Min float, Max float]
        ...
        # z,10 -> 10-3+1 -> z,8 -> max(3, 1) 8-3+1 -> z,6 -|
        if self.NetType == 'A':
            self.FC2_mu = nn.Linear(169, self.ActCase)    # default 2 out
            self.FC2_sigma = nn.Linear(169, self.ActCase)
        elif self.NetType == 'C':
            self.FC2_out = nn.Linear(169, 1)
    ...
    def GetPredictOut(self, x_py, x_comp):
        x = self._CommonPredictNet(x_py, x_comp)
        # x = nn.functional.hardtanh(self.FC2_A(x), self.ClipNetOut[0], self.ClipNetOut[1])
        # x = nn.functional.relu(self.FC2_A(x))
        # x = self.FC2_A(x)
        ...
        if self.NetType == 'A':
            x1 = 2.0 * torch.tanh(self.FC2_mu(x))
            x2 = nn.functional.softplus(self.FC2_sigma(x))
            x = (x1, x2)
        elif self.NetType == 'C':
            x = self.FC2_out(x)
        # x = nn.functional.log_softmax(self.FC2_A(x), dim=1)
    ...
    def TestOut(self, batchtest=False):
        ...
        CompTemp = tensor([[list(random(size=self.NubTimeSeq)) for _ in range(self.NubComPara)] for _ in range(Batch)],
                          dtype=torch.float)
        TOOL.ALLP(CompTemp)
        A = tensor([list(random(size=3)) for _ in range(Batch)], dtype=torch.float)
        TOOL.ALLP(A, comt='A')

        if self.NetType == 'A':
            (mu, sigma) = self.GetPredictOut(x_py=PhyTemp, x_comp=CompTemp)
            print(mu, sigma)
            dist = Normal(mu, sigma)
            AA = dist.sample()
            TOOL.ALLP(AA, comt="Act Sample")
            act_log_prob = dist.log_prob(A)
            TOOL.ALLP(act_log_prob, comt="Act_log_prob")

        elif self.NetType == 'C':
            value = self.GetPredictOut(x_py=PhyTemp, x_comp=CompTemp)
            print(value)
        # print(ra.randint(0, 10))
...
if __name__ == '__main__':
    for net_name in ["TEST", "Progno1"]:
        ...
        TESTMODEL = PPOModel2(name="AuxA", NetType='A', NubPhyPara=6, NubComPara=7, NubTimeSeq=15, ClipNetOut=[0, 25], ActCase=3)
        TESTMODEL.TestOut(batchtest=False)
        TESTMODEL.TestOut(batchtest=True)

        # TESTMODEL = PPOModel2(name="AuxC", NetType='C', NubPhyPara=6, NubComPara=7, NubTimeSeq=15, ClipNetOut=[-1, 1], ActCase=1)
        # TESTMODEL.TestOut(batchtest=False)
        # TESTMODEL.TestOut(batchtest=True)
        #
